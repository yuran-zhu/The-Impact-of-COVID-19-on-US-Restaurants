...
server <- shinyServer(function(input, output){
    ...
    #amount.2.plot.dataInput <- reactive({
     #   sub.1 <-  amount.2.dataInput() %>%
      #      filter(Date %in% c(as.Date("2020-10-26"), as.Date("2021-4-26")))
       # for (i in 1:nrow(sub.1 )) {
        #    sub.1[i,3] = sub.1[i,4]
        #} 
        #sub.2 <-  amount.2.dataInput() %>%
         #   filter(Date <= as.Date("2020-7-6") |  Date ==as.Date("2022-1-24"))
        #data <- rbind(sub.1, sub.2)
        #data
    #})
