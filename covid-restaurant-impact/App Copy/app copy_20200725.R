                      ...
                      tabItems(
                          ...
                          tabItem(tabName = "dashboard",
                                 ...
                                 # br(),
                                  ...
                                  tags$a(href="https://rallyforrestaurants.com/impact-COVID-19-restaurant-insights.html", "Restaurant Revenue & Dining Trend (From: Toast Plaform & Rally for Restaurant)"),br(),
                          ...
                          tabItem(tabName = "long",
                                  ...
                                  fluidRow(
                                      ...
                                      h4("Bad: Medium level of all seen during the pandemics"),
                                      br(),
...
server <- shinyServer(function(input, output){
    ...
    output$amountPlot.2 <- renderPlotly({
        amount.2.dataInput() %>%
            ...
            add_trace(data = amount.2.dataInput() [amount.2.dataInput() $Date >= as.Date("2020-7-6"),],
                      y = ~pred.1, name = '10% - 25%', mode = 'none', fill = 'tozeroy', fillcolor = '#c2c2d6') %>%  
            add_trace(y = ~pred.2, name = 'Stay at Home - Low', mode = 'none', fill = 'tozeroy', fillcolor = '#99e699') %>% 
        ...
        amount.3.dataInput() %>%
            ...
            layout(#title = 'Prediction of Total Revenues in 3 Months',
                   xaxis = list(title = "Date",
                                showgrid = FALSE),
                   yaxis = list(title = "Total Revenues",
                                showgrid = FALSE))  %>%  layout(legend = list(x = 0.1, y = 1))  # orientation = 'h'
        ...
        amount.4.dataInput() %>%
            ...
            layout(#title = 'Prediction of Total Revenues in 6 Months',
                   xaxis = list(title = "Date",
                                showgrid = FALSE),
                   yaxis = list(title = "Total Revenues",
                                showgrid = FALSE)) %>%  #layout(legend = list(orientation = 'h'))
            layout(showlegend = FALSE)
    })
    ...
    dine.in.pct.2.dataInput <- reactive({
        ...
        sub.2 <- amt.pct.new.total.dataInput() %>%
            filter(Date == as.Date("2021-4-26")) %>%
            filter(scenario == "gb") %>%
            select(1,2,3,6)  # 25 quantile for 6 month god bad
        colnames(sub.2)[4] <- "Dine_In_2020_weekly_pct"
     
        sub.3 <-  amt.pct.new.total.dataInput() %>%
            filter(Date == as.Date("2022-1-24")) %>%
            filter(scenario == "gg") %>%
            select(1,2,3,5)  # 10 quantile for 18 month good good
