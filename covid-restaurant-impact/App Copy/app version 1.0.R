...
## revenue data
revenue <- read.csv("cases_revenue_mob_46states.csv")
revenue$Date <- as.Date(as.character(revenue$Date))
revenue$Date.Week <- as.Date(as.character(revenue$Date.Week))

## weekly data
weekly.data <- read.csv("weekly_data_avg_full.csv")
                                  ...
                                  fluidRow(
                                      ...
                                      box(title = "Select State and Date", status = "warning", solidHeader = T,
                                          ...
                                          dateRangeInput('dateRange',
                                                         ...
                                                         start = as.Date("2020-2-17"), end = as.Date("2020-7-5"),
                                                         min = as.Date("2020-2-17"), max = as.Date("2020-7-5")),
                                          width = 3
                                      ),
...
server <- shinyServer(function(input, output){
    ...
    cor.weekly.dataInput <- reactive({
        if(input$var4 != "(All)") {
            ...
            for (j in 10:12) {
                for (i in seq(1, 140, 7)) {
                    us.total.mob[i,j] = sum(us.total.mob[i:(i+6),j-4]/7)
                }
            }
            data <- cbind(data, us.total.mob[seq(1, 140, 7), 10:12])
